...
class TestIoApic(pyobj.ConfObject):
    max_ints = 250
    def _initialize(self):
        super()._initialize()
        self.total = 0
        self.spikes_total = 0
        self.raised = [0] * self.max_ints
        self.spikes = [0] * self.max_ints

    class ioapic(pyobj.Interface):
        def eoi(self, vector):
            pass
        def interrupt(self, pin):
            if pin >= self._up.max_ints:
                stest.fail(f"unexpected pin number: {pin}")
            self._up.total+=1
            self._up.spikes_total+=1
            self._up.raised[pin]+=1
            self._up.spikes[pin]+=1
...
class HPE_TIMER:
    ...
    def __init__(self, endian = 'le', log_level = 1):
        ...
        self.endian = endian
        self.intr = simics.SIM_create_object('TestIoApic', 'test_apic', [])

        self.timer = simics.pre_conf_object('hpe_timer', self.dev_cls)
        self.timer.intc_apic = self.intr
        ...
        for reg in regs:
            ...
            self.regs[name] = f(self.timer.bank.regs, addr, 8)

    def adjust_log_level(self, new_level):
        self.timer.log_level = new_level

    def assert_intr_spikes(self, timN, expect):
        if self.intr.object_data.spikes_total != expect:
            raise Test_failure(
                "expected %d edge pulses but got %d in APIC"
                % (expect, self.intr.object_data.spikes_total))
    ...
    def assert_intr_state(self, timN, exp_apic, exp_pic = None):
        if self.intr.object_data.total == exp_apic:
            return
        raise Test_failure(
            "expected %d interrupts but got %d in APIC"
            % (exp_apic, self.intr.object_data.total))
    ...
    def reset(self, mode = 'HRESET'):
        ...
        port.signal_raise()
        port.signal_lower()
        self.intr.object_data.spikes_total = 0
