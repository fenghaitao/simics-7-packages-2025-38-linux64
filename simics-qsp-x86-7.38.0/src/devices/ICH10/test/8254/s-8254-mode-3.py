...
# s-8254-mode-3.py
# test the mode 3 triggering of the 8254 timer in the LPC bridge in the ICH9 chip

from timer_tb import *

def do_test(c):
    ctrl_val = (c << 6) + 0x16 # counter 0/1/2, LSB, mode 3
    ...
    SIM_continue(10)
    expect(tb.signal[c].level, 0,
           "output kept to 0 after counter %d is triggered" % c)

    SIM_continue(initial_cnt - 10)
    expect(tb.signal[c].level, 1,
           "output changed to 1 when counter %d is triggered secondly" % c)
