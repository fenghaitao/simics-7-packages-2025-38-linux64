...
# s-apic-eoi.py
# test the end-of-interrupt command of the APIC interrupt controller
# in the ICH9 chip
...
def do_test(intr_no):
    ...
    # Send the End-Of-Interrupt command to the APIC
    tb.write_value_le(apic_client_bank_base, 32, 0x4)

    # Check the interrupt request will be received in the pseudo APIC bus
    examine_intr_paras(intr_no,
                       test_vec, test_dest, test_dest_mode, test_deliv_mode)
    tb.apic_bus.current_interrupt = {}
    ...
    # Send the End-Of-Interrupt command again
    tb.write_value_le(apic_client_bank_base, 32, 0x4)
    expect_dict(tb.apic_bus.current_interrupt, {}, "no interrupt request anymore")
