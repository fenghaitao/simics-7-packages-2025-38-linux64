...
# s-smbus-word-data.py
# tests the word data transfer command of ICH9 SMBus controller module
...
def do_test():
    ...
    cmd_data1 = 0x88
    cmd_data2 = 0x99
    slave = tb.slaves[word_data_slave_addr]
    slave.caps_lock = 1
    slave.num_lock = 1
    ...
    tb.configure_transfer_paras(word_data_slave_addr, SmbusConst.smb_write,
                                cmd_code, cmd_data1, cmd_data2)
    # Set and start the smb command
    tb.start_smb_cmd(SmbusConst.cmd["word-data"])
    # Check the slave has toggled its capital lock state
    simics.SIM_continue(100)
    expect(slave.caps_lock, 0,
            "the capital lock state toggled in the word data SMBus slave device")
    expect(slave.num_lock, 1,
            "the number lock state kept in the word data SMBus slave device")
    expect(slave.cmd_code, cmd_code,
            "the command code written into the word data SMBus slave device")
    expect(slave.cmd_data1, cmd_data1,
            "the command data 1 written into the word data SMBus slave device")
    expect(slave.cmd_data2, cmd_data2,
            "the command data 2 written into the word data SMBus slave device")
    ...
    tb.configure_transfer_paras(word_data_slave_addr, SmbusConst.smb_read, cmd_code, 0, 0)
    # Set and start the smb command
    tb.start_smb_cmd(SmbusConst.cmd["word-data"])
    ...
    expect(slave.num_lock, 0,
            "the number lock state toggled in the word data SMBus slave device")
    expect(slave.cmd_code, cmd_code,
            "the command code written into the word data SMBus slave device")
    reg_val = tb.read_io_le(tb.smbus_func_mapped_addr + 0x5, 8)
    expect(reg_val, cmd_data1,
            "the command data 1 read from the word data SMBus slave device")
    reg_val = tb.read_io_le(tb.smbus_func_mapped_addr + 0x6, 8)
    expect(reg_val, cmd_data2,
            "the command data 2 read from the word data SMBus slave device")
