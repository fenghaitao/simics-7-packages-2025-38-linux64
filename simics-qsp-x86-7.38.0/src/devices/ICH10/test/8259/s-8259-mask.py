...
# s-8259-mask.py
# test the interrupt masking of the 8259A interrupt controller in the ICH9 chip

from tb_8259 import *

# Default initialization is to mask all interrupts
tb.init_i8259()

# Select all triggering mode to be level-triggering
tb.write_value_le(0x4D0, 8, 0xFF)
tb.write_value_le(0x4D1, 8, 0xFF)

def do_test(intr_no):
    other_intr = intr_no + 1
    if other_intr > 15:
        other_intr = 0
    tb.enable_interrupt(other_intr, 1)
    ...
    # Advance one step to allow the possible interrupt event to be triggered
    SIM_continue(1)

    # Check the interrupt service bit is not set
    if intr_no < 8:
        expect_hex((tb.read_irr_isr("isr", "master") >> intr_no) & 0x1, 0,
                   "interrupt service bit %d is not set" % intr_no)
    else:
        expect_hex((tb.read_irr_isr("isr", "slave") >> (intr_no - 8)) & 0x1, 0,
                   "interrupt service bit %d is not set" % intr_no)

    # Get the interrupt vector is expected void
    expect_list(tb.iack.current_interrupt, [],
                "void interrupt request got from the 8259")

    # Enable the interrupt
    tb.enable_interrupt(other_intr, 0)
    tb.enable_interrupt(intr_no, 1)

    # Advance one step to allow the possible interrupt event to be triggered
    SIM_continue(1)

    # Check the interrupt service bit is set now
    if intr_no < 8:
