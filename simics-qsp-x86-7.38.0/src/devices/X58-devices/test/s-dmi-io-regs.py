...
dmi = simics.SIM_create_object('x58-dmi', 'dmi', [["downstream_target",
                                                   dp.port.downstream]])
...
bf = dev_util.Bitfield_LE({'enable': 31,
                           ...
                           'fun': (10, 8),
                           'dword': (7, 2)})
config_address = dev_util.Register_LE(dmi.bank.io_regs, 0xcf8, 4, bf)
...
for sz in (4, 2, 1):
    for offs in range(0, 4, sz):
        config_data = dev_util.Register_LE(dmi.bank.io_regs, 0xcfc + offs, sz)
        ...
        if sz == 4:
            ...
            stest.expect_equal(dmi.bank.io_regs.config_data, exp_val)
            exp_val = random.randrange(1 << (sz * 8))
            dmi.bank.io_regs.config_data = exp_val
            got_val = int.from_bytes(img.iface.image.get(offs, sz), "little")
            stest.expect_equal(got_val, exp_val)
            simics.SIM_disable_breakpoint(bpid)
...
dmi.downstream_target = tstub
config_data = dev_util.Register_LE(dmi.bank.io_regs, 0xcfc, 4)
