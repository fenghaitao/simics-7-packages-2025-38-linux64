...
def do_test(domain_cnt, domain_id, domain_gaw, domain_space,
    ...
    for d in range(domain_cnt):
        ...
        base = tb.alloc_permanent_mem(table_size, VTdConst.page_size)
        ...
        ce_base = tb.alloc_permanent_mem(ce_size * device_cnt, VTdConst.page_size)
        ce_addr = ce_base
        for v in range(device_cnt):
            if v == VTdTestBench.vtd_dev_num:
                ce_val = 0
            else:
    ...
    re_addr = tb.alloc_permanent_mem(re_size, VTdConst.page_size)
    ...
    for d in range(domain_cnt):
        ...
        for v in range(device_cnt):
            ...
            read_bytes = vtd_hw_drv.issue_dma_remapping(0, v, 0,
                guest_addr, VTdHwDriver.dma_read, 4)
