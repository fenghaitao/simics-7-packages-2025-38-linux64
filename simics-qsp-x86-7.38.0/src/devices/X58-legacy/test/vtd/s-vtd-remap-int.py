...
# s-vtd-remap-int.py
# tests the remapping of interrupt request from guest software by
# the Intel (r) VT-d hardware
...
def do_test(pci_bus_id, device_id, func_id,
    ...
    if vtd_mode:
        ...
        if PRINT_DEBUG_INFO:
            print("Using Intel (r) VT-d spec for remapping")
    else:
        tb.vtd.use_vtd_interrupt_decoding = False
        if PRINT_DEBUG_INFO:
            print("Using X58 spec for remapping")
     ...
     vtd_mode
        ...
        (pci_bus, device, func,
         ...
         vtd_mode)
            ...
            for device in ((VTdTestBench._vtd_dev_num + 1) % 32,)# Device number on the PCI bus
