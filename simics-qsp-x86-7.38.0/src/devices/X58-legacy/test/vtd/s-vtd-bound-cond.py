...
# s-vtd-bound-cond.py
# tests some boundary conditions in the Intel (r) VT-d hardware
...
def test_pass_through(bus_id, dev_id, func_id,
    ...
    read_byte = vtd_hw_drv.issue_dma_remapping(bus_id, dev_id, func_id,
        guest_base + test_off, VTdHwDriver._dma_read, 1)
    stest.expect_equal(read_byte, (test_byte,),
    ...
    read_byte = vtd_hw_drv.issue_dma_remapping(bus_id, dev_id, func_id,
        guest_base + test_off, VTdHwDriver._dma_read, 1)
    stest.expect_equal(read_byte, (test_byte + 1,),
...
def test_disabled_int_remapping(bus_id, dev_id, func_id, int_vec, int_dest):
    ...
    # Intel 64 compatible request
    msg_addr = VTdConst.compat_int_msg_addr_bf.value(
                        FEE = 0xFEE,
                        DID = int_dest,
                    )
    msg_data = VTdConst.compat_int_msg_data_bf.value(
                        V   = int_vec,
                    )
    vtd_hw_drv.issue_int_message(bus_id, dev_id, func_id, msg_addr, msg_data)
    (vtd_destination, vtd_vector, vtd_tm, vtd_dm, vtd_dlm_mode) = \
...
def test_protected_read_write_fault(bus_id, dev_id, func_id,
    ...
    vtd_hw_drv.issue_dma_remapping(
        bus_id, dev_id, func_id, guest_base, VTdHwDriver._dma_write, 1, (1,))
    val = vtd_hw_drv.read_reg("FRCD%d" % frr_idx)
    fields = VTdConst.frcd_bf.fields(val)
    stest.expect_equal(fields['F'], 1, "fault present")
    stest.expect_equal(fields['FR'],
...
def do_test():
    ...
    one_addr    = one_space - VTdConst._page_size
    one_mapped  = tb.alloc_dynamic_mem(VTdConst._page_size)
    ...
    irte_size  = VTdConst._interrupt_entry_size
    ...
    test_pass_through(one_pci, one_dev, one_func, one_addr, one_mapped, ce_base)
    test_disabled_dma_remapping(one_pci, one_dev, one_func,
                                one_addr, one_mapped, ce_base)
    test_disabled_int_remapping(one_pci, one_dev, one_func,
                                one_vector, one_dest_cpu)
    test_no_present_ce_fault(one_pci, one_dev, one_func, ce_base)

    # Other than 4-level table walking fault
    test_unsupported_level_fault(one_pci, one_dev, one_func, ce_base)

    # Read/write protected fault
    test_protected_read_write_fault(one_pci, one_dev, one_func,
                                    one_addr, one_mapped, ce_base)

    # Fault overflow and fault buffer wrap around
    frr_cnt = VTdConst.cap_bf.fields(vtd_hw_drv.read_reg('CAP'))['NFR']
    frr_idx = get_next_frr()
    if frr_idx < frr_cnt:
        for i in range(frr_cnt - frr_idx):
            test_no_present_ce_fault(one_pci, one_dev, one_func, ce_base)
    frr_idx = get_next_frr()
    stest.expect_equal(frr_idx, 0,
                       "fault overflows and wraps around to the beginning")
    # Clear the fault record in the first recording register
    vtd_hw_drv.write_reg("FRCD0", VTdConst.frcd_bf.value(F = 1))
    test_no_present_ce_fault(one_pci, one_dev, one_func, ce_base)

def get_next_frr():
    frr_cnt = VTdConst.cap_bf.fields(vtd_hw_drv.read_reg('CAP'))['NFR']
    for i in range(frr_cnt):
