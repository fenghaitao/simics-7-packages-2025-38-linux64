...
# and interrupt entry caches in the Intel (r) VT-d hardware
...
def test_inv_cache(inv_if, inv_cache):
    ...
    one_len     = VTdConst._page_size # Must be at least one page
    ...
    irte_base  = tb.alloc_permanent_mem(irte_size * irte_cnt, VTdConst._page_size)
    irte_addr  = irte_base
    this_irte  = 0
    ...
    if inv_cache == cache_context:
        for (rw, g) in ((rw, g)
                        for rw in (VTdHwDriver._dma_read,
                                   VTdHwDriver._dma_write)
...
qbase = tb.alloc_permanent_mem(qsize * VTdConst._inv_descriptor_size)
