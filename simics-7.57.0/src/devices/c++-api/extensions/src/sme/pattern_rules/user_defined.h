...
#ifndef CPP_API_EXTENSIONS_SRC_SME_PATTERN_RULES_USER_DEFINED_H
#define CPP_API_EXTENSIONS_SRC_SME_PATTERN_RULES_USER_DEFINED_H
...
namespace sme {
     ...
     * 
     */
        class user_defined : public I_params_pattern_rule
        {
        public:
            /**
             ...
             * 
             */
            user_defined() : I_params_pattern_rule() {;}
            ...
            virtual void process_rule( uint64_t _old_value, uint64_t & _new_value) {
                if( is_active() && is_bound()) {
                    SIM_DEBUG_END( "true");
                    m_lambda( _old_value, _new_value);
                } else {
                    SIM_DEBUG_END( "false");
                }
            }
