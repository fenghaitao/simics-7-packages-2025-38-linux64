...
# This demonstrates how you can use pyobj to implement a device in Python with
# confclass.

import simics

class sample_device_python_confclass:
    """This device does nothing useful, but can be used as a starting point
       when writing Simics devices in Python.

       The source can be found at
       <tt>[simics]/src/devices/sample-device-python-confclass/</tt>."""

    # Create a conf-class with the name 'sample_device_python_confclass'.
    cls = simics.confclass('sample_device_python_confclass',
                           short_doc='one-line doc for the class',
                           doc='longer doc for the class')

    # Create an temperature attribute.
    cls.attr.temperature("i", default=0, doc="The temperature register.")

    # Implement the 'io_memory' iface.
    ...
    def operation(self, mop, info):
        ...
        if offset == 0x00:
            ...
            if simics.SIM_mem_op_is_read(mop):
                simics.SIM_set_mem_op_value_le(mop, 0)
            simics.SIM_log_error(self.obj, 0, "Illegal offset 0x%x, "
