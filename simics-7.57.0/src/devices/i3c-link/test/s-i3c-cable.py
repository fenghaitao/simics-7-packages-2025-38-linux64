...
clk.freq_mhz = 25.0
clk.cell = cell
simics.SIM_add_configuration([clk, cell, domain], None)
clk = conf.clk
...
for d in [ep0, ep1, cable, master, slave]:
    d.queue = clk
...
master.bus = ep0
slave.bus = ep1
simics.SIM_add_configuration([cable, ep0, ep1, master, slave], None)
...
def test_ibi():
    ...
    master.wait('ibi_address', 0xa1, lambda: slave.ibi_address(0xa1))
    slave.wait('ibi_acknowledge', I3C_noack, lambda: master.ibi_acknowledge(I3C_noack))
    slave.wait('stop', 1, lambda: master.stop())
    master.wait('ibi_request', 1, lambda: slave.ibi_request())
    slave.wait('ibi_start', 1, lambda: master.ibi_start())
    master.wait('ibi_address', 0xa2, lambda: slave.ibi_address(0xa2))
    slave.wait('ibi_acknowledge', I3C_ack, lambda: master.ibi_acknowledge(I3C_ack))
    slave.wait('read', 1, lambda: master.read())
    master.wait('read_response', 0xaa, lambda: slave.read_response(0xaa, False))
    slave.wait('stop', 1, lambda: master.stop())
