...
# Test master HDR write transaction
...
m_h_ep.hdr_write(bytes([0x11 << 1, 0 << 7 | 0x63]))  # 0x11 is the HDR command code, 0x63 cmd]))
c()
expect([s, m], [['hdr_write', bytes([0x11 << 1, 0 << 7 | 0x63])], []])
